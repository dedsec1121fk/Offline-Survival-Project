...
class OfflineDatabase:
    ...
    def integrity_report(self) -> dict[str, Any]:
        """Validate the consolidated database, bilingual parity, dates, field types, and source provenance."""
        report: dict[str, Any] = {
            "database_file": str(self.database_file),
            "physical_json_files": 1 if self.database_file.is_file() else 0,
            "languages": {},
            "paths_match": False,
            "ids_match": False,
        ...
        for language in LANGUAGES:
            ...
            try:
                groups = self.language_payload(language).get("source_groups", [])
            except Exception:
                groups = []
                invalid_files += 1
            if not isinstance(groups, list):
            ...
            relative_paths = set(ids_by_file)
        ...
        report["ok"] = bool(
            report["physical_json_files"] == 1
            and report["paths_match"]
            and report["ids_match"]
            and report["file_ids_match"]
            and all(
                values["root_present"]
                and values["source_groups"] > 0
                and all(
                    values[key] == 0
                    for key in (
                        "invalid_files",
...
class Application:
    ...
    def record_label(self, item: dict[str, Any]) -> str:
        ...
        meta = item.get("search_meta") if isinstance(item.get("search_meta"), dict) else {}
        if meta:
            fields = ", ".join(str(v) for v in meta.get("matched_fields", [])[:3])
            extras = []
            if meta.get("fuzzy_terms"):
                extras.append("typo/fuzzy" if self.language == "en" else "ανοχή ορθογραφίας")
            if meta.get("cross_language_terms"):
                extras.append("cross-language" if self.language == "en" else "διαγλωσσικό")
            detail = " · ".join(x for x in [fields, *extras] if x)
            return f"{title} [{category}]" + (f" — {detail}" if detail else "")
    ...
    def record_lines(self, item: dict[str, Any]) -> list[str]:
        ...
        detail = record_detail_profile(record)
        lines.append("")
        lines.extend(wrap_lines("Detailed record map" if self.language == "en" else "Αναλυτικός χάρτης εγγραφής"))
        lines.append("·" * terminal_width())
        lines.extend(self.render_value_lines(detail))
    ...
    def file_actions(self, selected: tuple[str, list[dict[str, Any]]]) -> None:
        ...
        while True:
            ...
            if command == "1":
                ...
                self.view_raw_file(relative_path)
    ...
    def view_raw_file(self, relative_path: str) -> None:
        try:
            raw = self.database.raw_file_json(self.language, relative_path)
            lines: list[str] = []
...
def print_cli_help() -> None:
    ...
    print('\nValidate both languages inside the consolidated database and return exit code 0/2:')
        ...
        if len(sys.argv) > 1:
            ...
            if command == "--stats":
                ...
                for language, values in report["languages"].items():
                    print(
                        ...
                        f"{values['records']} records, {values['source_groups']} source groups, "
            ...
            if command in {"--ui-test", "--ui-logic-test"}:
                ...
                if not node:
                    ...
                    return 2
                # MAINTENANCE: Keep the deterministic JavaScript harness embedded so the
...
def _consolidated_embedded_tool_source(name: str, source: str) -> str:
    """Adapt legacy audit internals to the single physical database JSON file.

    MAINTENANCE: The quality algorithms are preserved; only their record-loading
    adapter changes. The old virtual source-group paths remain available for
    diagnostics and pairing without requiring hundreds of physical JSON files.
    """
    if name == "content_quality":
        old = '''def load_refs(lang: str) -> list[RecordRef]:
    refs: list[RecordRef] = []
    for path in sorted((ROOT / lang).rglob("*.json")):
        try:
            doc = json.loads(path.read_text(encoding="utf-8"))
    ...
    return refs
'''
        new = '''def load_refs(lang: str) -> list[RecordRef]:
    refs: list[RecordRef] = []
    db_path = ROOT / "Offline Survival Database.json"
    try:
        document = json.loads(db_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError):
        return refs
    code = "en" if lang == "English" else "el"
    language = document.get("languages", {}).get(code, {}) if isinstance(document, dict) else {}
    groups = language.get("source_groups", []) if isinstance(language, dict) else []
    if not isinstance(groups, list):
        return refs
    for group in groups:
        if not isinstance(group, dict):
        ...
        if not isinstance(records, list):
            continue
        virtual_path = ROOT / "__virtual_database__" / lang / str(group.get("path", "group.json"))
        for i, record in enumerate(records):
            if isinstance(record, dict):
                refs.append(RecordRef(lang, virtual_path, records, records, i, record))
    return refs
'''
        if old not in source:
            raise RuntimeError("Could not adapt content-quality loader to consolidated database")
        source = source.replace(old, new, 1)
    elif name == "translation_audit":
        old = '''def load_records(root: Path) -> dict[str, dict]:
    out: dict[str, dict] = {}
    for path in sorted(root.rglob("*.json")):
        data = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(data, list):
    ...
    return out
'''
        new = '''def load_records(root: Path) -> dict[str, dict]:
    out: dict[str, dict] = {}
    db_path = ROOT / "Offline Survival Database.json"
    data = json.loads(db_path.read_text(encoding="utf-8"))
    code = "en" if root.name == "English" else "el"
    language = data.get("languages", {}).get(code, {}) if isinstance(data, dict) else {}
    groups = language.get("source_groups", []) if isinstance(language, dict) else []
...
def _consolidated_web_source(source: str) -> str:
    ...
    old = """        for code in ("en", "el"):
            ...
            add(f"database_{code}", root.is_dir(), str(root))
"""
    new = """        database_file = PROJECT_ROOT / "Offline Survival Database.json"
        add("consolidated_database", database_file.is_file(), str(database_file))
        for code in ("en", "el"):
            try:
                DATABASE.language_payload(code)
                add(f"database_{code}", True, "inside consolidated database")
            except Exception as error:
                add(f"database_{code}", False, str(error))
"""
    if old not in source:
...
def _phone_assets_test() -> tuple[bool, str]:
    ...
    shell_files = sorted(PROJECT_ROOT.rglob("*.sh"))
    ok = not missing and engine_neutral and not shell_files
    detail = "single-script installed/default-browser path present" if ok else f"missing={missing}, engine_neutral={engine_neutral}, shell_files={len(shell_files)}"
    return ok, detail
...
def _standalone_reader_test() -> int:
    ...
    ck("chapter-sequence", ids == list(range(1, 251)), f"{ids[:3]}...{ids[-3:] if ids else []}")
    ...
    ck("smart-reader-search", "smartChapterScore" in text and "SEARCH_GROUPS" in text and "editDistance" in text)
    ck("reader-chapter-map", "Chapter map:" in text and "Χάρτης κεφαλαίου:" in text)
...
def integrated_self_test() -> int:
    ...
    required = [
        ...
        PROJECT_ROOT / "Offline Survival Database.json",
    ...
    check("no-shell-scripts", not sh_files, ", ".join(str(p.relative_to(PROJECT_ROOT)) for p in sh_files))
    ...
    typo_rows = database.search("en", "waterr storag")
    check("smart-search-typo-tolerance", bool(typo_rows) and "water" in normalize(typo_rows[0]["record"].get("title", "")) and "storag" in normalize(typo_rows[0]["record"].get("title", "")), typo_rows[0]["record"].get("title", "") if typo_rows else "no result")
    cross_rows = database.search("el", "water storage")
    check("smart-search-cross-language", bool(cross_rows) and "νερ" in normalize(cross_rows[0]["record"].get("title", "")), cross_rows[0]["record"].get("title", "") if cross_rows else "no result")
    roman_rows = database.search("el", "nero apothikefsi")
    check("smart-search-greek-transliteration", bool(roman_rows) and "νερ" in normalize(roman_rows[0]["record"].get("title", "")), roman_rows[0]["record"].get("title", "") if roman_rows else "no result")
    filtered_rows = database.search("en", "priority:high blackout -generator")
    ...
    # MAINTENANCE: Keep the numbered bilingual compendium contiguous. If chapters are
    # intentionally added or removed, update this range and regenerate the standalone reader.
    compendium = PROJECT_ROOT / "Offline Library" / "Knowledge Compendium"
    chapter_sets = {}
    for folder in ("EN", "GR"):
        numbers = []
        for chapter in (compendium / folder).glob("*.md"):
            match = re.match(r"^(\d+)-", chapter.name)
            if match and int(match.group(1)) > 0:
                numbers.append(int(match.group(1)))
        chapter_sets[folder] = sorted(numbers)
    expected_chapters = list(range(1, 251))
    check("knowledge-compendium-en-sequence", chapter_sets.get("EN") == expected_chapters, f"{len(chapter_sets.get('EN', []))}/250")
    check("knowledge-compendium-gr-sequence", chapter_sets.get("GR") == expected_chapters, f"{len(chapter_sets.get('GR', []))}/250")
...
def integrated_deep_audit() -> int:
    ...
    # MAINTENANCE: Read every text Library file line-by-line. Binary reference packs are
    # covered by hashing/duplicate checks and are deliberately not decoded as UTF-8 here.
    library_text_files = []
    library_line_count = 0
    library_root = PROJECT_ROOT / "Offline Library"
    for path in library_root.rglob("*") if library_root.is_dir() else []:
        if not path.is_file() or path.suffix.lower() not in {".md", ".txt", ".csv"}:
            continue
        library_text_files.append(path)
        try:
            raw = path.read_bytes()
            if bytes([0]) in raw:
                findings.append(f"NUL byte in Library text: {path.relative_to(PROJECT_ROOT)}")
                continue
            text = raw.decode("utf-8")
        except Exception as exc:
            findings.append(f"Unreadable Library UTF-8: {path.relative_to(PROJECT_ROOT)}: {exc}")
            continue
        library_line_count += text.count("\n") + 1
        for no, line in enumerate(text.splitlines(), 1):
            if line.rstrip() != line:
                findings.append(f"Trailing whitespace in Library: {path.relative_to(PROJECT_ROOT)}:{no}")
            if "\ufffd" in line:
                findings.append(f"Replacement character in Library: {path.relative_to(PROJECT_ROOT)}:{no}")
    ...
    repository_json=sorted(PROJECT_ROOT.rglob("*.json"))
    if repository_json != [PROJECT_ROOT / "Offline Survival Database.json"]:
        findings.append("Expected exactly one repository JSON file: Offline Survival Database.json")
    ...
    print(f"Source/config/document lines inspected: {line_count}")
    print(f"Library text files inspected: {len(library_text_files)}")
    print(f"Library text lines inspected: {library_line_count}")
    print(f"Total text lines inspected: {line_count + library_line_count}")
    print(f"JSON/config files parsed: {json_count}")
    print(f"Python scripts: {len(py_files)}")
    print(f"Shell scripts: {len(sh_files)}")
    print(f"Repository JSON files: {len(repository_json)}")
    if findings:
