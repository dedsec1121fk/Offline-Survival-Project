...
It combines a curated survival database, a phone-friendly local Command Center, a large bilingual Offline Library, a 220-subject Knowledge Compendium, planning and tracking tools, and a completely standalone survival reader.
...
* **220 detailed Knowledge Compendium subjects per language.**
...
* `python "Offline Survival.py" --check` — validate both language databases and mirrored IDs/paths.
...
The repository intentionally ships with only one Python script:

* `Offline Survival.py`

That single script contains the Python-side functionality for:
...
The Knowledge Compendium contains **220 detailed subjects in English and 220 corresponding subjects in Greek**.
...
* The repository intentionally keeps **one Python script only**.
* Standard JSON does not support `//` or `#` comments.
* Technical JSON files therefore use valid maintenance metadata fields where appropriate instead of invalid JSON comments.
* `MAINTENANCE.json` records project-wide maintenance rules.
...
Συνδυάζει επιμελημένη βάση γνώσεων, τοπικό Command Center για κινητό, μεγάλη δίγλωσση Offline Library, Knowledge Compendium 220 θεμάτων, εργαλεία σχεδιασμού και καταγραφής και έναν πλήρως αυτόνομο οδηγό επιβίωσης.
...
* **220 αναλυτικά θέματα Knowledge Compendium ανά γλώσσα.**
* **871 επιμελημένες εγγραφές βάσης ανά γλώσσα.**
* **732 αρχεία Offline Library** με δίγλωσσα ζεύγη και τοπική αναζήτηση πλήρους κειμένου.
...
Το repository περιέχει σκόπιμα μόνο ένα Python script:
...
Το Knowledge Compendium περιέχει **220 αναλυτικά θέματα στα Αγγλικά και 220 αντίστοιχα θέματα στα Ελληνικά**.
...
* Το repository κρατά σκόπιμα **μόνο ένα Python script**.
* Το standard JSON δεν υποστηρίζει `//` ή `#` comments.
* Τα τεχνικά JSON χρησιμοποιούν έγκυρα maintenance metadata fields όπου χρειάζεται.
* Το `MAINTENANCE.json` περιέχει τους project-wide maintenance κανόνες.
