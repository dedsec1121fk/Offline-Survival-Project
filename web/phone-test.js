...
(() => {
  ...
  const T = {
    en:{
      eyebrow:'OFFLINE SURVIVAL PROJECT · V7', title:'Phone Browser Diagnostics', intro:'Run this page inside the browser actually installed on your phone. Results stay on this device and are not uploaded.',
      ...
      privacyText:'Privacy: the diagnostic report contains browser/device capability information and the local app version. It is generated locally and is downloaded only if you choose Export.', rawHeading:'Raw local report',
    ...
    el:{
      eyebrow:'OFFLINE SURVIVAL PROJECT · V7', title:'Διαγνωστικός έλεγχος browser τηλεφώνου', intro:'Τρέξε αυτή τη σελίδα μέσα στον browser που είναι πραγματικά εγκατεστημένος στο τηλέφωνό σου. Τα αποτελέσματα μένουν στη συσκευή και δεν ανεβαίνουν πουθενά.',
      ...
      privacyText:'Απόρρητο: η διαγνωστική αναφορά περιέχει πληροφορίες δυνατοτήτων browser/συσκευής και την τοπική έκδοση της εφαρμογής. Δημιουργείται τοπικά και κατεβαίνει μόνο αν επιλέξεις Εξαγωγή.', rawHeading:'Ακατέργαστη τοπική αναφορά',
  ...
  async function run(){
    ...
    const checks=[];
    try{ const meta=await fetchJson('/api/meta'); checks.push(row('meta',status(meta && Number(meta.version)>=7),`v${meta.version} · schema ${meta.schema}`)); }catch(e){ checks.push(row('meta','fail',e.message)); }
    ...
    try{ const paths=['/styles.css','/app.js','/v5.js','/v6.js','/v7.js']; const sizes=[]; for(const p of paths){ const t=await fetchText(p); sizes.push(`${p}:${t.length}`); } checks.push(row('shell','pass',sizes.join(' · '))); }catch(e){ checks.push(row('shell','fail',e.message)); }
    ...
    latest={
      generated_at:new Date().toISOString(), language:lang, project:'Offline Survival Project', command_center_expected:7,
