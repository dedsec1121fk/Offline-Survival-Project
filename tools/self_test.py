#!/usr/bin/env python3
# MAINTENANCE: This is the structural release gate. Add checks whenever a new persistent module or static asset is introduced.
"""Offline Survival Project self-test.
...
def main() -> int:
    required = [
        ...
        WEB / "styles.css",
        WEB / "app.js",
        WEB / "field-operations.js",
        WEB / "continuity-operations.js",
        WEB / "knowledge-atlas.js",
        WEB / "phone-test.html",
        WEB / "phone-test.js",
        WEB / "sw.js",
    ...
    field_js = (WEB / "field-operations.js").read_text(encoding="utf-8")
    continuity_js = (WEB / "continuity-operations.js").read_text(encoding="utf-8")
    knowledge_js = (WEB / "knowledge-atlas.js").read_text(encoding="utf-8")
    js = app_js + "\n" + field_js + "\n" + continuity_js + "\n" + knowledge_js
    ...
    static_map = {"/styles.css": WEB / "styles.css", "/app.js": WEB / "app.js", "/field-operations.js": WEB / "field-operations.js", "/continuity-operations.js": WEB / "continuity-operations.js", "/knowledge-atlas.js": WEB / "knowledge-atlas.js", "/manifest.webmanifest": WEB / "manifest.webmanifest"}
    ...
    try:
        maintenance = json.loads((ROOT / "MAINTENANCE.json").read_text(encoding="utf-8"))
        manifest_note = manifest.get("_maintenance_comment", "") if isinstance(manifest, dict) else ""
        maintenance_ok = bool(maintenance.get("_maintenance_comment")) and bool(maintenance.get("_maintenance_comment_el")) and bool(manifest_note)
        check("maintenance-json-metadata", maintenance_ok, "valid JSON maintenance metadata present" if maintenance_ok else "missing maintenance metadata")
    except Exception as error:
        check("maintenance-json-metadata", False, str(error))
    ...
    # MAINTENANCE: The state schema is internal compatibility metadata, not a public release number.
    schema_match = re.search(r"SCHEMA_VERSION\s*=\s*(\d+)", server)
    check("state-schema-defined", bool(schema_match), "internal state compatibility schema present" if schema_match else "missing SCHEMA_VERSION")
    ...
    check("browser-backup-schema", bool(export_schemas) and all(x == expected_schema for x in export_schemas), "browser backups match internal compatibility schema")
    ...
    if node:
        for asset in (WEB / "app.js", WEB / "field-operations.js", WEB / "continuity-operations.js", WEB / "knowledge-atlas.js", WEB / "phone-test.js", WEB / "sw.js", ROOT / "tools" / "ui_logic_test.js"):
...
'''
        proc = run([node, "-e", probe, str(WEB / "app.js"), str(WEB / "field-operations.js"), str(WEB / "continuity-operations.js"), str(WEB / "knowledge-atlas.js")])
        ...
        check("ui-logic-runtime", ui_proc.returncode == 0, (ui_proc.stderr or ui_proc.stdout)[-700:] if ui_proc.returncode else "DOM/state logic exercised without network")
        if proc.returncode != 0:
            pass
    else:
    ...
    print("Offline Survival Project — self-test")
    print("=" * 64)
